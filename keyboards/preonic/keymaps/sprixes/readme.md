...
## 🛠️ Compiling, Flashing, and Linting

This guide outlines how to lint, compile, and flash this layout onto a **Planck** or **Preonic** keyboard. Run these commands from the root directory of your `qmk_firmware` repository.

### 1. Linting (Syntax & Configuration Checks)
To check the keymap configuration for errors or potential improvements without compiling, run:

#### Using QMK CLI:
```bash
# For Planck
qmk lint -kb planck/rev6 -km sprixes

# For Preonic
qmk lint -kb preonic/rev3 -km sprixes
```

---

### 2. Compiling (Building the Firmware)
To build the firmware binary (`.bin` or `.hex`) for the layout:

#### Using QMK CLI:
```bash
# For Planck
qmk compile -kb planck/rev6 -km sprixes

# For Preonic
qmk compile -kb preonic/rev3 -km sprixes
```

#### Using Classic `make`:
```bash
# For Planck
make planck/rev6:sprixes

# For Preonic
make preonic/rev3:sprixes
```

---

### 3. Flashing (Uploading to Keyboard)
To compile and immediately flash the keyboard. (Note: Put your keyboard into bootloader mode first, e.g. tap the `QK_BOOT` key under the `_ADJUST` layer or press the physical reset button on the bottom of the PCB.)

#### Using QMK CLI:
```bash
# For Planck
qmk flash -kb planck/rev6 -km sprixes

# For Preonic
qmk flash -kb preonic/rev3 -km sprixes
```

#### Using Classic `make`:
```bash
# For Planck
make planck/rev6:sprixes:dfu-util

# For Preonic
make preonic/rev3:sprixes:dfu-util
```
